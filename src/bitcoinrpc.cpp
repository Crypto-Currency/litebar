...
// Copyright (c) 2013 The LiteBar Developers
...
Value stop(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "LiteBar server has now stopped running!";
}
...
// LiteBar: Return average network hashes per second based on last number of blocks.
...
Value setaccount(const Array& params, bool fHelp)
{
    ...
    if (!address.IsValid())
        throw JSONRPCError(-5, "Invalid LiteBar address");
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        ...
        if (!address.IsValid())
            throw JSONRPCError(-5, string("Invalid LiteBar address:")+s.name_);
...
Value encryptwallet(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "wallet encrypted; LiteBar server stopping, restart to run with encrypted wallet";
}
...
Value getworkex(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "LiteBar is not connected!!!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "LiteBar is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (params.size() == 0)
    {
        ...
        result.push_back(Pair("target",   HexStr(BEGIN(hashTarget), END(hashTarget))));
        result.push_back(Pair("algorithm", "scrypt:1024,1,1"));  // LiteBar: specify that we should use the scrypt algorithm
        return result;
    }
    else
    {
